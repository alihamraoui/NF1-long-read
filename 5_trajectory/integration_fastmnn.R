####### file from https://github.com/satijalab/seurat-wrappers/blob/master/R
...
#'
#' @importFrom Seurat DefaultAssay DefaultAssay<- SelectIntegrationFeatures VariableFeatures
#' as.SingleCellExperiment CreateDimReducObject Tool<- LogSeuratCommand CreateAssayObject LogSeuratCommand
#' @importFrom SingleCellExperiment reducedDim rowData
...
#'
integration_fastmnn = function(object.list,
                               assay = NULL,
                               features = 2000,
                               reduction.name = "mnn",
                               reduction.key = "mnn_",
  ...
  assay = assay %||% Seurat::DefaultAssay(object = object.list[[1]])

  for (i in 1:length(x = object.list)) {
    Seurat::DefaultAssay(object = object.list[[i]]) = assay
  }
  ...
  objects.sce = lapply(X = object.list,
                       FUN = function(x, f) {
                         return(Seurat::as.SingleCellExperiment(x = subset(x = x, features = f)))
                       },
                       f = features)
  ...
  out = do.call(what = batchelor::fastMNN,
                args = c(
                  objects.sce,
                  list(...)))
  ...
  integrated[[reduction.name]] = Seurat::CreateDimReducObject(embeddings = SingleCellExperiment::reducedDim(x = out),
                                                              loadings = as.matrix(SingleCellExperiment::rowData(x = out)),
                                                              assay = Seurat::DefaultAssay(object = integrated),
                                                              key = reduction.key)

  # Add reconstructed matrix (gene x cell)
  integrated[[reconstructed.assay]] = Seurat::CreateAssayObject(data = as(object = SummarizedExperiment::assay(x = out),
                                                                          Class = "sparseMatrix"))
  # Add variable features
  Seurat::VariableFeatures(object = integrated[[reconstructed.assay]]) = features
  Seurat::Tool(object = integrated) = S4Vectors::metadata(x = out)
  integrated = Seurat::LogSeuratCommand(object = integrated)
